...
public partial class QRCodeGenerator : IDisposable
{
    ...
    /// <exception cref="BliveHelper.Utils.QRCoder.Exceptions.DataTooLongException">Thrown when the payload is too big to be encoded in a QR code.</exception>
    ...
    public static QRCodeData GenerateQrCode(string plainText, ECCLevel eccLevel, bool forceUtf8 = false, bool utf8BOM = false, EciMode eciMode = EciMode.Default, int requestedVersion = -1)
    {
        ...
        else
        {
            ...
            if (minVersion > version)
            {
                ...
                throw new BliveHelper.Utils.QRCoder.Exceptions.DataTooLongException(eccLevel.ToString(), encoding.ToString(), version, maxSizeByte);
            }
        }
    ...
    private static readonly BitArray _repeatingPattern = new([true, true, true, false, true, true, false, false, false, false, false, true, false, false, false, true]);
    ...
    private static QRCodeData GenerateQrCode(BitArray bitArray, ECCLevel eccLevel, int version)
    {
        ...
        List<CodewordBlock> CalculateECCBlocks()
        {
            ...
            void AddCodeWordBlocks(int blocksInGroup, int codewordsInGroup, int offset, Polynom generatorPolynom)
            {
                var groupLength = codewordsInGroup * 8;
                for (var i = 0; i < blocksInGroup; i++)
                {
                    var eccWordList = CalculateECCWords(bitArray, offset, groupLength, eccInfo, generatorPolynom);
                    codewordBlocks.Add(new CodewordBlock(offset, groupLength, eccWordList));
                    offset += groupLength;
                }
            }
        }
    ...
    private static readonly BitArray _getFormatGenerator = new([true, false, true, false, false, true, true, false, true, true, true]);
    private static readonly BitArray _getFormatMask = new([true, false, true, false, true, false, false, false, false, false, true, false, false, true, false]);
    /// <summary>
    ...
    private static readonly BitArray _getVersionGenerator = new([true, true, true, true, true, false, false, true, false, false, true, false, true]);
    ...
    private static BitArray PlainTextToBinaryNumeric(string plainText)
    {
        ...
        else if (plainText.Length % 3 == 1)  // One remaining digit is encoded in 4 bits.
        {
            ...
            DecToBin(dec, 4, bitArray, index);
        }

        return bitArray;
    }

    private static readonly Encoding _iso8859_1 = Encoding.GetEncoding(28591); // ISO-8859-1
    private static Encoding _iso8859_2;

    public static BitArray RepeatingPattern => _repeatingPattern;
